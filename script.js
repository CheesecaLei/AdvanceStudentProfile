...
function makeItRain() {
    ...
    for (let i = 0; i < numElements; i++) {
        setTimeout(() => {
            createMagicalElement(emojiElements);
        }, i * 150);
    }
}

function createMagicalElement(emojiElements) {
    const element = document.createElement('div');
    element.className = 'heart';
    element.textContent = emojiElements[Math.floor(Math.random() * emojiElements.length)];
...
function updateGreeting() {
    ...
    if (hour < 12) {
        ...
        greeting = "🌙 Good Evening! Thanks for visiting my profile!";
    }
...
// ACTIVITY 2 - DARK/LIGHT MODE TOGGLE
...
// ACTIVITY 2 - CONTACT FORM VALIDATION
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize Activity 2 features
    updateGreeting();
    initNavigation();
    initThemeToggle();
    initContactForm();
